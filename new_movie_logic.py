...
def view_member_info():
    ...
    if user_phone in customers:
        ...
        if 'bookings' in user_info: 
            ...
            global showtime, selected_format, selected_movie, theater, booking_details, booking_list, booking_key
...
def checkout():
    ...
    if o_ask == 'y':
        ...
        global otp
        otp = 234 #MS.sending_OTP()

        global stored_cvv
        stored_cvv = user_info.get('CVV_num')

        otp_ask = int(input("Please enter the OTP given to you: ").strip())

        if otp_ask == otp:
            calculate_total_amount(ticket_prices, user_info)
            cvv_enter = input("Please input your CVV Number to verify: ").strip()
            if cvv_enter != stored_cvv:
                print("Sorry! CVV number does not match! You are locked out! ")
            
                sys.exit()
            else:
                print(f"Success! Payment has been made with credit card number!")
                print(f"Thank you for choosing Sai's cinemas! ")
                MS.confirm_ticket(selected_format, selected_movie, showtime, theater)

                # Iterate over each booking and call confirm_ticket for each one
...
def calculate_total_amount(ticket_prices, user_info):
    ...
    if 'bookings' in user_info:
        for booking_key, booking_list in user_info['bookings'].items():
            selected_format = booking_key.split('-')[1]

            for booking_details in booking_list:
                ticket_price = ticket_prices.get(selected_format, 0)
                total_amount += ticket_price

    GST = decimal.Decimal('0.08')

    subtotal = total_amount

    convenience_fee = decimal.Decimal('2')

    gst_amount = GST * subtotal

    final_price = subtotal + gst_amount + convenience_fee

    print(f"Subtotal: ${subtotal:.2f}\n")
    print(f"GST Payable($): ${gst_amount:.2f}\n")
    print(f"Convenience Fees: ${convenience_fee:.2f}\n")
    print(f"Final Price Payable: ${final_price:.2f}\n")
...
def main(): 
    ...
    while True: 
        ...
        print("4. View Member Info/Checkout")
